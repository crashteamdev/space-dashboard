    ...
    try {
      ...
      axiosApiInstance
        ...
        .then((response) => {
          dispatch(
            addItem({
              title: errorHandler(response.status, {
      ...
      dispatch(
        addItem({
          title: errorHandler(err.response.status, {
            four: "На вашем аккаунте не достаточно средств для покупки тарифа"
          }),
          status: "error",
          timelife: 4000,
...
export const getExchange = (token: string, currency: string) => async (dispatch: AppDispatch) => {
  try {
    const config = {
      method: "get",
      maxBodyLength: Infinity,
